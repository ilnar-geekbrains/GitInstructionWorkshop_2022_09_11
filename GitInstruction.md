...
Репозиторий это папка на локальном компьютере в которой произведена инициализация git-а командой:  
***`git init`***
Эта команда создает скрытую папку `.git`  с дочерними папками[^1] в которых и будет производиться сохранение версий файлов в репозитории. Удаление папки `.git` равнозначно удалению репозитория.
...
Команда `git commit -m <"название коммита">` - создает коммит (точку фиксации изменений) с описанием <"название коммита">, заключать название кавычки обязательно.  
Если файл уже был добавлен командой `git add` можно при создании коммита использовать команду `git commit -am <"название коммита">`.

Команда `git checkout <номер коммита>` - позволяет перейти к версии файла зафиксированой конкретным коммитом. Номер коммита можно посмотреть с помощью команды `git log`, или её краткой версией `git log --oneline`. Перед созданием коммита файл должен быть сохранен.
...
Репозиторий на внешнем устройстве, нужен для совместного доступа.
...
[^1]: Именно по этой причине нельзя создавать репозиторий в папке с другим репозиторем.