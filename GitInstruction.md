...
для того чтобы скопировать внешний репозиторий из сети на компьютер, необходимо использовать команду git clone [ссылка на репозиторий].

### git push — переносим изменения в удаленный репозиторий
для того чтобы получать изменения и обновлять рабочую копию так, чтобы она соответствовала удаленному репозиторию. И для этого в Git существует отдельная команда. Называется она git pull.

### git pull — забираем изменения из центрального репозитория
Команда git pull позволяет скачать все изменения и автономатически делает слияние с активной веткой. 
