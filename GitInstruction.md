...
Ветки в git чаще всего служат для удобного выполнения отдельных задач.

Так при возникновении необходимости создания новой задачи, или добавления какой-либо фичи в проект создается отдельная ветка. Впоследствии, после того как работа над этой задачей окончена, ветка с ее изменениями вливается или как говорят мержится (merge), в основную (main) ветку проекта. Данный способ позволяет внести готовые и рабочие измения сразу, вместо постепенного добавления отдельных строк.

 Использование веток позволяет упорядочить работу, а также делает весь процесс разработки более наглядным и понятным для других пользователей.
