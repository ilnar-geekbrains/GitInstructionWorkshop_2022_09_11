...
Для инициализации репозитория необходимо создать папку на локальном компьютере и открыть папку через комманду быстрого доступа:
```
CTRL+K
CTRL+O
```
Или во вкладке меню **файл => Открыть папку...**.

Теперь необходимо ввести в терминале команду:
```
git init
```

### 2. Создание файла в репозитории
Для начала необходимо находиться в папке в которой планируется создание файла.
Для создания файла можно воспользоваться сочетанием клавиш `__CTRL + N__` или в меню `__Файл => Создать текстовый файл/Создать файл__`. Не забываем при создании файла указывать его расширение.
